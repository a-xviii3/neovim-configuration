...
Keymap("v", "<M-j>", ":m '>+1<CR>gv=<ESC>gv", { desc = "core/base: move current selection up" })
Keymap("v", "<M-k>", ":m '<-2<CR>gv=<ESC>gv", { desc = "core/base: move current selection down" })

-- stationary cursor
NKeymap("J", "mzJ`z", { desc = "core/base: don't move the cursor when joining the next line" })
NKeymap("<C-d>", "<C-d>zz", { desc = "core/base: keep cursor centered when scrolling down" })
NKeymap("<C-u>", "<C-u>zz", { desc = "core/base: keep the cursor at the center when scrolling up" })
NKeymap("n", "nzzzv", { desc = "core/base: keep cursor centered while navigating search results" })
NKeymap("N", "Nzzv", { desc = "core/base: keep cursor centered while navigating search results" })

-- modification/extensions of existing keymaps
Keymap({ "n", "v" }, "x", "\"_x", { desc = "core/base: remove without copying" })
...
NKeymap("<leader>C", "\"_C", { desc = "core/base: delete till eol and edit without copying" })
